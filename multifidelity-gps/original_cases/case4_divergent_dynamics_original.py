...
def y_HF(t):
    return np.sin(20*t) + t + 0.3*t**2 - 0.2*t**3  # Polynomial + trigonometric

def y_LF(t):
    return (0.9*np.sin(19*t) + 0.7*t + 0.4*np.exp(-2*t)*np.sin(15*t) + 
            0.2*np.cos(8*t)/(1 + 2*t))  # Exponential + trigonometric
...
def standard_gp_co_kriging(N_HF, N_LF, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.set_title(f'Case 4: Divergent Dynamics | Standard GP | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}', 
                      fontsize=11, pad=10)
        ...
        # ax2.scatter(t_train_HF, delta_train, s=40, color='#9467bd', marker='s', 
...
def joint_gp_co_kriging(N_HF, N_LF, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.legend(loc='best', mode="expand", ncol=2, fontsize=9)
        ax1.text(0.02, 0.99, 'a', transform=ax1.transAxes, fontsize=14, fontweight='bold', va='top')
        ax1.set_title(f'Case 4: Divergent Dynamics | Joint GP | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}', 
                      fontsize=11, pad=10)
...
#%%    
    ...
    ax.set_title('Case 4: Divergent Dynamics | GP Methods Comparison: RMSE vs Data Quantity', fontsize=13, pad=15)
