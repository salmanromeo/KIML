...
def y_HF(t):
    return np.sin(20*t) + t + 0.2*np.sin(20*t)*np.cos(10*t)  # Multiplicative terms!

def y_LF(t):
    return (0.8*np.sin(18*t) + 0.6*t + 0.4 + 
            0.15*np.sin(18*t)*np.sin(8*t) + 0.1*t*np.cos(12*t))
...
def standard_gp_co_kriging(N_HF, N_LF, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.set_title(f'Case 2: Moderate Nonlinearities | Standard GP | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}', 
                      fontsize=11, pad=10)
...
def joint_gp_co_kriging(N_HF, N_LF, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.legend(loc='best', mode="expand", ncol=2, fontsize=9)
        ax1.text(0.02, 0.99, 'a', transform=ax1.transAxes, fontsize=14, fontweight='bold', va='top')
        ax1.set_title(f'Case 2: Moderate Nonlinearities | Joint GP | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}', 
                      fontsize=11, pad=10)
...
#%%    
    ...
    ax.set_title('Case 2: Moderate Nonlinearities | GP Methods Comparison: RMSE vs Data Quantity', fontsize=13, pad=15)
