...
def y_HF(t):
    if isinstance(t, tf.Tensor):
        return tf.sin(20.0 * t) + t
    else:
        return np.sin(20 * t) + t

def y_LF(t):
    if isinstance(t, tf.Tensor):
        return 0.8 * tf.sin(20.0 * t) + 0.5 * t + 0.5
    else:
...
def standard_NN(N_HF, N_LF, physics_weight=0.5, physics_std=10.0, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.set_title(f'Case 1: Linear Scaling | Standard NN | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}',
                      fontsize=11, pad=10)
...
def ansatz_NN(N_HF, N_LF, physics_weight=0.5, physics_std=10.0, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.legend(loc='best', mode="expand", ncol=2, fontsize=9)
        ax1.text(0.02, 0.99, 'a', transform=ax1.transAxes, fontsize=14, fontweight='bold', va='top')
        ax1.set_title(f'Case 1: Linear Scaling | Ansatz NN | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}',
                      fontsize=11, pad=10)
...
def bayesian_ansatz_NN(N_HF, N_LF, noise_std=0.1, n_chains=1, num_results=80, num_burnin_steps=20, plot_results=True):
    ...
    if plot_results:
        ...
        ax1.legend(loc='best', mode="expand", ncol=2, fontsize=9)
        ax1.text(0.02, 0.99, 'a', transform=ax1.transAxes, fontsize=14, fontweight='bold', va='top')
        ax1.set_title(f'Case 1: Linear Scaling | Bayesian Ansatz NN | $N_{{HF}}$={N_HF} | RMSE: {rmse:.4f} | Complexity: {complexity}', 
                     fontsize=11, pad=10)
...
if __name__ == "__main__":
    ...
    ax.set_title('Case 1: Linear Scaling | NN Methods Comparison: RMSE vs Data Quantity', 
                fontsize=13, pad=15)
